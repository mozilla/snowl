...
const EXPORTED_SYMBOLS = ["FeedWriter"];
...
FeedWriter.prototype = {
  ...
  _appInfo: Cc["@mozilla.org/xre/app-info;1"].
  ...
  _setContentText: function FW__setContentText(id, text) {
    this._contentSandbox.element = this._elements[id];
  ...
  _setTitleImage: function FW__setTitleImage(container) {
    try {
      ...
      var feedTitleImage = this._elements["feedTitleImage"];
      this._safeSetURIAttribute(feedTitleImage, "src", 
                                parts.getPropertyAsAString("url"));

      // Set up the title image link
      var feedTitleLink = this._elements["feedTitleLink"];
      ...
      this._contentSandbox.feedTitleText = this._elements["feedTitleText"];
...
leafName = this._appInfo.name + ".exe";
}
//#else
//#ifdef XP_MACOSX
else if (this._appInfo.OS == "Darwin") {
//#expand           if (fp.file.leafName != "__MOZ_APP_DISPLAYNAME__.app") {
// FIXME: figure out how to get the equivalent of __MOZ_APP_DISPLAYNAME__
// in JavaScript. Perhaps something in the user agent or hanging off navigator?
leafName = /* something + */ ".app";
}
//#else
else {
//#expand           if (fp.file.leafName != "__MOZ_APP_NAME__-bin") {
leafName = this._appInfo.name + "-bin";
}
//#endif
//#endif
if (fp.file.leafName != leafName) {
  ...
  _setAlwaysUseCheckedState: function FW__setAlwaysUseCheckedState(feedType) {
    var checkbox = this._elements["alwaysUse"];
    if (checkbox) {
      var alwaysUse = false;
      try {
  ...
  _setSubscribeUsingLabel: function FW__setSubscribeUsingLabel() {
    ...
    this._contentSandbox.subscribeUsing =
      this._elements["subscribeUsingDescription"];
  ...
  _setAlwaysUseLabel: function FW__setAlwaysUseLabel() {
    var checkbox = this._elements["alwaysUse"];
    if (checkbox) {
      var handlersMenuList = this._elements["handlersMenuList"];
      if (handlersMenuList) {
    ...
    if (event.type == "command") {
      switch (event.target.getAttribute("anonid")) {
        ...
        case "chooseApplicationMenuItem":
           ...
           */
          var popupbox = this._elements["handlersMenuList"]
  ...
  _setSelectedHandler: function FW__setSelectedHandler(feedType) {
    ...
    switch (handler) {
      ...
      case "bookmarks":
      default: {
        var liveBookmarksMenuItem = this._elements["liveBookmarksMenuItem"];
  ...
  _initSubscriptionUI: function FW__initSubscriptionUI() {
    var handlersMenuPopup = this._elements["handlersMenuPopup"];
    ...
    this._elements["subscribeButton"]
  ...
  init: function FW_init(aWindow, aFeedWindow, aElements, aFeed) {
    ...
    this._feed = aFeed;
    this._feedURI = this._feed.machineURI;
    if (!this._feedURI)
      return;
    ...
    LOG("Subscribe Preview: feed uri = " + this._feed.machineURI.spec);
  ...
  close: function FW_close() {
    this._elements["handlersMenuPopup"]
  ...
  subscribe: function FW_subscribe() {
    ...
    var useAsDefault = this._elements["alwaysUse"]
    ...
    if (selectedItem.hasAttribute("webhandlerurl")) {
      ...
      if (handler) {
        ...
        this._feedWindow.location.href = handler.getHandlerURI(this._feedURI.spec);
      }
    }
    else {
      ...
      // FIXME: figure out what these are in the new world.
      var feedTitle = this._feed.name;
      var feedSubtitle = this._feed.subtitle;
      feedService.addToClientReader(this._feed.machineURI.spec, feedTitle, feedSubtitle, feedType);
    }
